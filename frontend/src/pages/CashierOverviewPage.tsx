...
import { alpha, Box, Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, InputAdornment, Paper, Stack, Table, TableBody, TableCell, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
export function CashierOverviewPage() {
  const { closeCash, error, isCashOpen, loading, movements, openCash, refreshCashSession, sales, session, summary } = useCashSession();
  ...
  const cardDifferenceNumber = countedCardNumber - summary.expectedCardNumber;
  const totalDifferenceNumber = cashDifferenceNumber + pixDifferenceNumber + cardDifferenceNumber;
  ...
  const overviewRows = useMemo(() => [
    ...sales.map((sale) => ({ id: `sale-${sale.id}`, sortDate: sale.createdAt, registerType: "Venda", code: sale.code, description: "Venda no balcão", detail: sale.paymentMethodLabel, timeLabel: sale.timeLabel, value: sale.total, tone: "#1c6d25" })),
    ...movements.map((movement) => ({ id: `movement-${movement.id}`, sortDate: movement.createdAt, registerType: "Movimentação", code: movement.code, description: movement.description || movement.typeLabel, detail: movement.typeLabel, timeLabel: movement.timeLabel, value: movement.value, tone: movementToneColor(movement.type) })),
  ].sort((left, right) => new Date(right.sortDate).getTime() - new Date(left.sortDate).getTime()), [movements, sales]);
  ...
  function handleOpenCloseDialog() {
    ...
    setCardCountedInput(formatMoneyInput(summary.expectedCardNumber));
  ...
  async function handleOpenCash() {
    try {
      ...
      await openCash(parseMoneyInput(openingFundInput)); setOpeningFundInput("");
    } catch (requestError) {
  ...
  async function handleCloseCash() {
    try {
      setActionLoading(true); setPageError(null);
      await closeCash({ cashCounted: countedCashNumber, pixCounted: countedPixNumber, cardCounted: countedCardNumber });
  ...
  async function handleCreateMovement() {
    ...
    try {
      setActionLoading(true); setPageError(null);
      await createCashMovement({ type: movementDialogMode, value: parseMoneyInput(movementValueInput), description: movementDescription.trim() });
      await refreshCashSession();
      setMovementDialogMode(null); setMovementValueInput(""); setMovementDescription("");
    } catch (requestError) {
      setPageError(requestError instanceof Error ? requestError.message : "Não foi possível registrar a movimentação.");
    } finally { setActionLoading(false); }
  }

  if (loading) return <Box sx={{ minHeight: "60vh", display: "grid", placeItems: "center" }}><CircularProgress size={32} /></Box>;
      ...
      <Stack spacing={3}>
        <Paper elevation={0} sx={{ p: { xs: 2.5, md: 3 }, borderRadius: "12px", bgcolor: alpha("#ffffff", 0.78), backdropFilter: "blur(12px)" }}>
          <Stack direction="row" spacing={1.25} alignItems="center">
            ...
            <Box><Typography variant="h4">Caixa</Typography><Typography color="text.secondary">Status do PDV: Caixa Fechado</Typography></Box>
          </Stack>
        </Paper>
        <Box sx={{ minHeight: "calc(100vh - 240px)", display: "grid", placeItems: "center" }}>
          <Paper elevation={0} sx={{ width: "min(100%, 460px)", p: { xs: 3, md: 4 }, borderRadius: "24px", bgcolor: alpha("#ffffff", 0.9), backdropFilter: "blur(14px)", boxShadow: "0 24px 60px rgba(45, 52, 51, 0.08)" }}>
            <Stack spacing={3} alignItems="center" textAlign="center">
              <Box sx={{ width: 78, height: 78, display: "grid", placeItems: "center", borderRadius: "24px", bgcolor: "rgba(157, 241, 151, 0.34)", color: "primary.main" }}><PointOfSaleRoundedIcon sx={{ fontSize: 40 }} /></Box>
              <Box><Typography variant="h4" sx={{ mb: 1 }}>Abertura de Caixa</Typography><Typography color="text.secondary">Inicie o turno definindo o valor inicial para troco e habilite as comandas e as vendas no caixa.</Typography></Box>
              {pageError || error ? <Typography color="error" sx={{ fontWeight: 700 }}>{pageError ?? error}</Typography> : null}
              <TextField fullWidth label="Fundo de Troco Inicial" placeholder="0,00" value={openingFundInput} onChange={(event) => setOpeningFundInput(event.target.value)} sx={{ "& .MuiOutlinedInput-root": { minHeight: 68, borderRadius: "14px", bgcolor: "#eef3f1" } }} InputProps={{ startAdornment: <InputAdornment position="start"><Typography sx={{ fontWeight: 800, color: "primary.main" }}>R$</Typography></InputAdornment> }} />
              <Button fullWidth disabled={actionLoading} onClick={() => void handleOpenCash()} size="large" startIcon={<LockOpenRoundedIcon />} sx={{ minHeight: 58, borderRadius: "14px", background: "linear-gradient(135deg, #1c6d25 0%, #9df197 100%)", color: "#083f10" }} variant="contained">Abrir Caixa</Button>
            </Stack>
          </Paper>
        </Box>
      </Stack>
  ...
  return (
    <>
      <Stack spacing={3}>
        <Paper elevation={0} sx={{ p: { xs: 2.5, md: 3 }, borderRadius: "12px", bgcolor: alpha("#ffffff", 0.78), backdropFilter: "blur(12px)" }}>
          <Stack direction={{ xs: "column", lg: "row" }} spacing={2} justifyContent="space-between" alignItems={{ xs: "flex-start", lg: "center" }}>
            <Box>
              <Stack direction="row" spacing={1.25} alignItems="center" sx={{ mb: 1 }}><PointOfSaleRoundedIcon color="secondary" /><Typography variant="h4">Caixa Aberto</Typography></Stack>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", xl: "minmax(0, 1fr) 360px" }, gap: 3 }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))", xl: "repeat(4, minmax(0, 1fr))" }, gap: 2 }}>
              {summaryCards.map((card) => (
                <Paper elevation={0} key={card.label} sx={{ p: 2.5, borderRadius: "12px", bgcolor: "background.paper", boxShadow: "0 12px 28px rgba(45, 52, 51, 0.05)" }}>
                  <Stack spacing={1.25}>{card.icon}<Typography color="text.secondary" sx={{ fontSize: "0.82rem" }}>{card.label}</Typography><Typography variant="h5">{card.value}</Typography><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>{card.hint}</Typography></Stack>
                </Paper>
              ))}
            </Box>
            <Paper elevation={0} sx={{ p: { xs: 2.5, md: 3 }, borderRadius: "12px", bgcolor: "background.paper", boxShadow: "0 12px 28px rgba(45, 52, 51, 0.05)" }}>
              <Stack spacing={2}>
                <Box><Typography variant="h5" sx={{ mb: 0.5 }}>Resumo Operacional do Dia</Typography><Typography color="text.secondary">Todas as vendas e movimentações do caixa aparecem juntas aqui.</Typography></Box>
                <Table size="small">
                  <TableHead><TableRow><TableCell>Registro</TableCell><TableCell>Código</TableCell><TableCell>Descrição</TableCell><TableCell>Identificação</TableCell><TableCell>Horário</TableCell><TableCell align="right">Valor</TableCell></TableRow></TableHead>
                  <TableBody>
                    {overviewRows.length === 0 ? <TableRow><TableCell colSpan={6} sx={{ color: "text.secondary" }}>Nenhum registro operacional no dia até o momento.</TableCell></TableRow> : overviewRows.map((row) => (
                      <TableRow key={row.id}><TableCell>{row.registerType}</TableCell><TableCell>{row.code}</TableCell><TableCell>{row.description}</TableCell><TableCell>{row.detail}</TableCell><TableCell>{row.timeLabel}</TableCell><TableCell align="right" sx={{ fontWeight: 800, color: row.tone, whiteSpace: "nowrap" }}>{row.value}</TableCell></TableRow>
                    ))}
                  </TableBody>
                </Table>
              </Stack>
          ...
          <Paper elevation={0} sx={{ p: { xs: 2.5, md: 3 }, borderRadius: "12px", bgcolor: "#eef3f1", display: "flex", flexDirection: "column", gap: 2.5 }}>
            <Box><Typography variant="h5" sx={{ mb: 0.5 }}>Movimentações do Caixa</Typography><Typography color="text.secondary">Registros reais da sessão aberta no backend.</Typography></Box>
            <Stack spacing={1.5}>
              {movements.length === 0 ? <Typography color="text.secondary">Nenhuma movimentação registrada até o momento.</Typography> : movements.map((movement) => (
                <Paper elevation={0} key={movement.id} sx={{ p: 2, borderRadius: "10px", bgcolor: "background.paper" }}>
                  <Stack direction="row" spacing={1.5} justifyContent="space-between" alignItems="center">
                    <Box sx={{ minWidth: 0 }}><Typography variant="subtitle2" sx={{ mb: 0.4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{movement.description || movement.typeLabel}</Typography><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>{movement.code} - {movement.timeLabel}</Typography></Box>
      ...
      <Dialog fullWidth maxWidth="sm" onClose={() => setCloseCashDialogOpen(false)} open={closeCashDialogOpen}>
        ...
        <DialogContent>
          <Stack spacing={2.5} sx={{ pt: 1 }}>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(3, minmax(0, 1fr))" }, gap: 1.5 }}>
              ...
              <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "#eef3f1" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Esperado em Cartão</Typography><Typography variant="h6">{summary.expectedCard}</Typography></Paper>
            </Box>
            <Stack spacing={2}>
              ...
              <TextField label="Cartão conferido" value={cardCountedInput} onChange={(event) => setCardCountedInput(event.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Typography sx={{ fontWeight: 800, color: "primary.main" }}>R$</Typography></InputAdornment> }} />
            </Stack>
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" }, gap: 1.5 }}>
              ...
              <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "#f7f9f8" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença em Cartão</Typography><Typography sx={{ fontWeight: 800, color: differenceTone(cardDifferenceNumber) }}>{brl.format(cardDifferenceNumber)}</Typography></Paper>
