...
export function CashierOverviewPage() {
    ...
    if (!isCashOpen) {
        return <Stack spacing={3}><Box><h1 style={{ margin: 0, color: "#4a76d6", fontSize: "clamp(2rem, 3vw, 3rem)", lineHeight: 1, letterSpacing: "-0.04em", fontFamily: '"Plus Jakarta Sans", sans-serif', fontWeight: 800 }}>Caixa</h1><Typography color="text.secondary" sx={{ mt: 2.5 }}>Status do PDV: Caixa Fechado</Typography></Box><Box sx={{ minHeight: "calc(100vh - 240px)", display: "grid", placeItems: "center" }}><Paper elevation={0} sx={{ width: "min(100%, 460px)", p: { xs: 3, md: 4 }, borderRadius: "24px", bgcolor: "var(--surface-float)", backdropFilter: "blur(14px)", boxShadow: "var(--shadow-soft)", border: "1px solid", borderColor: "divider" }}><Stack spacing={3} alignItems="center" textAlign="center"><Box sx={{ width: 78, height: 78, display: "grid", placeItems: "center", borderRadius: "24px", bgcolor: "rgba(157, 241, 151, 0.34)", color: "primary.main" }}><PointOfSaleRoundedIcon sx={{ fontSize: 40 }} /></Box><Box><Typography variant="h4" sx={{ mb: 1 }}>Abertura de Caixa</Typography><Typography color="text.secondary">Inicie o turno definindo o valor inicial para troco e habilite as comandas e as vendas no caixa.</Typography></Box>{pageError || error ? <Typography color="error" sx={{ fontWeight: 700 }}>{pageError ?? error}</Typography> : null}<TextField fullWidth label="Fundo de Troco Inicial" placeholder="0,00" value={openingFundInput} onChange={(event) => setOpeningFundInput(event.target.value)} sx={{ "& .MuiOutlinedInput-root": { minHeight: 68, borderRadius: "14px", bgcolor: "var(--surface-soft)" } }} InputProps={{ startAdornment: <InputAdornment position="start"><Typography sx={{ fontWeight: 800, color: "primary.main" }}>R$</Typography></InputAdornment> }} /><Button fullWidth disabled={actionLoading} onClick={() => void handleOpenCash()} size="large" startIcon={<LockOpenRoundedIcon />} sx={{ minHeight: 58, borderRadius: "14px", background: "linear-gradient(135deg, #1c6d25 0%, #9df197 100%)", color: "#083f10" }} variant="contained">Abrir Caixa</Button></Stack></Paper></Box></Stack>;
    }
        ...
        <>
            <Stack spacing={3}>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", xl: "minmax(0, 1fr) 360px" }, gap: 3 }}>
                    ...
                    <Paper elevation={0} sx={{ p: { xs: 2.5, md: 3 }, borderRadius: "12px", bgcolor: "var(--surface-soft)", display: "flex", flexDirection: "column", gap: 2.5 }}>
                        <Box>
                            <Typography variant="h5" sx={{ mb: 0.5 }}>Movimentações do Caixa</Typography>
                            <Typography color="text.secondary">Registros reais da sessão aberta.</Typography>
                        </Box>
                        <Stack spacing={1.5}>
                            {movements.length === 0 ? (
                ...
                <DialogContent>
                    <Stack spacing={2.5} sx={{ pt: 1 }}>
                        ...
                        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))", xl: "repeat(4, minmax(0, 1fr))" }, gap: 1.5 }}>
                            ...
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Esperado em Débito</Typography><Typography variant="h6">{summary.expectedDebit}</Typography></Paper>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Esperado em Crédito</Typography><Typography variant="h6">{summary.expectedCredit}</Typography></Paper>
                        </Box>
                        ...
                        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))", xl: "repeat(5, minmax(0, 1fr))" }, gap: 1.5 }}>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença em Dinheiro</Typography><Typography sx={{ fontWeight: 800, color: differenceTone(cashDifferenceNumber) }}>{brl.format(cashDifferenceNumber)}</Typography></Paper>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença em Pix</Typography><Typography sx={{ fontWeight: 800, color: differenceTone(pixDifferenceNumber) }}>{brl.format(pixDifferenceNumber)}</Typography></Paper>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença em Débito</Typography><Typography sx={{ fontWeight: 800, color: differenceTone(debitDifferenceNumber) }}>{brl.format(debitDifferenceNumber)}</Typography></Paper>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença em Crédito</Typography><Typography sx={{ fontWeight: 800, color: differenceTone(creditDifferenceNumber) }}>{brl.format(creditDifferenceNumber)}</Typography></Paper>
                            <Paper elevation={0} sx={{ p: 2, borderRadius: "12px", bgcolor: "var(--surface-soft)" }}><Typography color="text.secondary" sx={{ fontSize: "0.8rem" }}>Diferença Total</Typography><Typography sx={{ fontWeight: 900, color: differenceTone(totalDifferenceNumber) }}>{brl.format(totalDifferenceNumber)}</Typography></Paper>
                        </Box>
                    </Stack>
                </DialogContent>
                <DialogActions sx={{ px: 3, pb: 2.5 }}>
